...
.hero-title-line1 { color: #f8fafc; display: block; }
.hero-title-line2 {
    display: block;
...
palette_m   = ["#7c3aed","#0891b2","#10b981","#f59e0b","#ef4444"]
# ══════════════════════════════════════════
st.markdown("""
<div class="sec-wrap">
  <div class="sec-chip">🎯 Validation</div>
  <div class="sec-heading">ROC Curves &amp; Confusion Matrix</div>
  <div class="sec-rule"></div>
</div>
""", unsafe_allow_html=True)
...
with cr:
    ...
    for i, nm in enumerate(model_names):
        ...
        fig_roc.add_trace(go.Scatter(x=fpr, y=tpr, mode="lines",
            name=f"{nm} · {results[nm]['auc']:.3f}",
            line=dict(color=palette_m[i], width=2.2 if nm==best_name else 1.5)))
    fig_roc.add_shape(type="line",x0=0,y0=0,x1=1,y1=1,
        line=dict(color="rgba(255,255,255,0.08)",dash="dash"))
    fig_roc.update_layout(
        title="ROC Curves", xaxis_title="False Positive Rate", yaxis_title="True Positive Rate",
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font_color="#475569", title_font_color="#94a3b8", title_font_size=13,
        legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(size=10, color="#475569")),
...
with cc:
    ...
    fig_cm = px.imshow(cm, text_auto=True,
        x=["Predicted DOWN","Predicted UP"], y=["Actual DOWN","Actual UP"],
        color_continuous_scale=[[0,"#0a0f1a"],[0.5,"#1e1050"],[1,"#7c3aed"]],
...
st.markdown("""
<div class="sec-wrap">
  <div class="sec-chip">🔬 Explainability</div>
  <div class="sec-heading">Feature Importance</div>
  <div class="sec-rule"></div>
</div>
""", unsafe_allow_html=True)

try:
    rf_mdl = results[best_name]["pipeline"].named_steps["model"]
    imps   = rf_mdl.feature_importances_
    n      = min(len(imps), len(num_cols))
    fi_df  = pd.DataFrame({"Feature": num_cols[:n], "Importance": imps[:n]})
    fi_df  = fi_df.sort_values("Importance").tail(12)

    fig_fi = px.bar(fi_df, x="Importance", y="Feature", orientation="h",
        color="Importance",
        color_continuous_scale=[[0,"#1e1050"],[0.5,"#7c3aed"],[1,"#06b6d4"]],
        title=f"Top Predictive Features · {best_name}")
    fig_fi.update_traces(marker_line_width=0)
    fig_fi.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font_color="#475569", height=400, title_font_color="#94a3b8", title_font_size=13,
        coloraxis_showscale=False,
        xaxis=dict(gridcolor="rgba(255,255,255,0.04)"),
        yaxis=dict(tickfont=dict(color="#94a3b8")))
    st.plotly_chart(fig_fi, use_container_width=True)
    st.info("💡 RSI, MACD, Volatility, and Fear & Greed Index are the strongest predictors of tomorrow's crypto price direction.")
except Exception:
...
st.markdown("""
<div class="sec-wrap">
  <div class="sec-chip">📊 Data Insights</div>
  <div class="sec-heading">Exploratory Analysis</div>
  <div class="sec-rule"></div>
</div>
""", unsafe_allow_html=True)
...
with ea1:
    btc = df[df["symbol"]=="BTC"].reset_index(drop=True).tail(180)
    fig_p = go.Figure()
    for i in range(1, len(btc)):
        c = "#10b981" if btc["close"].iloc[i] >= btc["close"].iloc[i-1] else "#ef4444"
        fig_p.add_trace(go.Scatter(x=[i-1,i], y=[btc["close"].iloc[i-1],btc["close"].iloc[i]],
            mode="lines", line=dict(color=c,width=1.8), showlegend=False))
    fig_p.update_layout(title="BTC Price Trend (Last 180 Days)",
        xaxis_title="Day", yaxis_title="Price ($)", **PLOT_CFG)
    st.plotly_chart(fig_p, use_container_width=True)

with ea2:
    avg = df.groupby("symbol")["close"].mean().reset_index().sort_values("close",ascending=False)
    fig_a = px.bar(avg, x="symbol", y="close", color="symbol",
        title="Average Close Price by Coin",
        color_discrete_sequence=palette_m)
    fig_a.update_traces(marker_line_width=0, opacity=0.85)
    fig_a.update_layout(showlegend=False, **PLOT_CFG)
    st.plotly_chart(fig_a, use_container_width=True)

eb1, eb2 = st.columns(2)

with eb1:
    fig_rsi2 = px.histogram(df, x="rsi_14", color="symbol", nbins=40,
        title="RSI Distribution by Coin", barmode="overlay", opacity=0.65,
        color_discrete_sequence=palette_m)
    fig_rsi2.add_vline(x=70, line_dash="dash", line_color="#ef4444",
        annotation_text="Overbought", annotation_font_color="#ef4444")
    fig_rsi2.add_vline(x=30, line_dash="dash", line_color="#10b981",
        annotation_text="Oversold", annotation_font_color="#10b981")
    fig_rsi2.update_layout(**PLOT_CFG)
    st.plotly_chart(fig_rsi2, use_container_width=True)

with eb2:
    fg_c2 = df["feargreed_class"].value_counts().reset_index()
    fg_c2.columns = ["class","count"]
    order2 = ["Extreme Fear","Fear","Neutral","Greed","Extreme Greed"]
    fg_c2["class"] = pd.Categorical(fg_c2["class"], categories=order2, ordered=True)
    fig_fg2 = px.bar(fg_c2.sort_values("class"), x="class", y="count", color="class",
        title="Fear & Greed Sentiment Distribution",
        color_discrete_sequence=["#ef4444","#f97316","#64748b","#22c55e","#10b981"])
    fig_fg2.update_traces(marker_line_width=0, opacity=0.85)
    fig_fg2.update_layout(showlegend=False, **PLOT_CFG)
    st.plotly_chart(fig_fg2, use_container_width=True)
...
with ec1:
    ...
    tgt.columns = ["dir","cnt"]
    tgt["dir"] = tgt["dir"].map({1:"📈 UP",0:"📉 DOWN"})
    fig_pie = px.pie(tgt, names="dir", values="cnt",
        title="Target Distribution  UP vs DOWN",
...
with ec2:
    fig_sc = px.scatter(df.sample(1000, random_state=1),
        x="volatility_30d", y="rsi_14", color="symbol",
        title="Volatility vs RSI", opacity=0.55,
        color_discrete_sequence=palette_m)
    fig_sc.update_layout(**PLOT_CFG)
    st.plotly_chart(fig_sc, use_container_width=True)

# Heatmap
corr = df.select_dtypes(include="number").drop(columns=["target"]).corr()
fig_h = px.imshow(corr, text_auto=".2f", aspect="auto",
    title="Feature Correlation Heatmap",
    color_continuous_scale=[[0,"#1e0a3c"],[0.5,"#0d1b2a"],[1,"#064e3b"]],
    zmin=-1, zmax=1)
fig_h.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
    font_color="#475569", title_font_color="#94a3b8", title_font_size=13, height=500,
    coloraxis=dict(colorbar=dict(tickfont=dict(color="#475569"))))
